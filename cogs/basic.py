...
class Basic(commands.Cog):
    ...
    async def on_command_error(self, ctx, ex):
        print(ex)
        await ctx.send("Please check with i?help the usage of this command or talk to your administrator.")

    @commands.command(brief="Any message to owo")
    async def owo(self, ctx):
        await ctx.send(text_to_owo(ctx.message.content[-6:]))
    ...
    @commands.command(brief="Poke someone by sending a dm from bot!")
